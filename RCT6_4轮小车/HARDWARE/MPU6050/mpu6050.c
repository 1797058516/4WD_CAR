...
//声明外部二值信号量
extern xSemaphoreHandle MPU6050_Bin_SemaphoreHandle;
/* MPU6050的中断 */
/**
 * @name: EXTI15_10_IRQHandler
 * @msg: MPU6050的中断 MPU6050_Bin_SemaphoreHandle信号量置1任务读取一次MPU数据
 * @return {*}
 */
void EXTI15_10_IRQHandler(void)
{
    BaseType_t xpxHigherPriorityTaskWoken, err;
    if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_14) == 0)
    {
        err = xSemaphoreGiveFromISR(MPU6050_Bin_SemaphoreHandle, &xpxHigherPriorityTaskWoken);
        if (err != pdPASS)
        {
            printf("give from isr binsema error\r\n");
        }
    }
