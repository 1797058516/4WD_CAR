...
void Usart2_DMA_Init(void)
{
    ...
    // Usart2_Handle.USART2_TX_FLAG = 0;
}
...
void u2_printf(char *format, ...)
{
    ...
    // va_list ap;
    // va_start(ap, format);
    // vsprintf((char *)Usart2_Handle.USART2_TX_BUF, format, ap);
    // va_end(ap);
    // i = strlen((const char *)Usart2_Handle.USART2_TX_BUF); //此次发送数据的长度
    // // DMA_USART2_Tx_Data(Usart2_Handle.USART2_TX_BUF,strlen((const char*)Usart2_Handle.USART2_TX_BUF));	//发送USART2_TX_BUF内容

    // for (j = 0; j < i; j++)                                //循环发送数据
    // {
    //     while (USART_GetFlagStatus(USART2, USART_FLAG_TC) == RESET)
    //         ; //循环发送,直到发送完毕
    //     USART_SendData(USART2, Usart2_Handle.USART2_TX_BUF[j]);
    // }
}
...
//         DMA_USART2_Tx_Data(p, USART2_MAX_RECV_LEN);
